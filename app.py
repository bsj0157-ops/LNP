...
import lnp_model_v5 as M5  # 💡 V5 모듈 임포트
    ...
    if not PDF_OK:
        ...
        if up:
            with st.spinner("PDF 읽는 중..."):
                try: ex = LP.extract(io.BytesIO(up.read()))
                except Exception as e: st.error(f"추출 실패: {e}"); ex = None
            if ex:
                c1, c2, c3 = st.columns(3)
                c1.metric("페이지", ex["n_pages"])
                c2.metric("조성 후보", len(ex["ratios"]))
                c3.metric("EE 후보", len(ex.get("ee_specific", [])))
                doi = st.text_input("DOI (확인/수정)", value=ex["doi"] or "")
                st.subheader("찾은 조성 후보")
                for i, r in enumerate(ex["ratios"]):
                    with st.expander(f"{r['ratio']}   (p.{r['page']}, 합계 {r['sum']})", expanded=(i == 0)):
                        st.code(r["evidence"], language=None)
                st.divider()
                st.subheader("표 편집 후 추가")
                draft = LP.to_draft_rows(ex)
                if doi: draft["reference_doi"] = doi.strip().lower()
                ...
                if st.button("이 행들을 데이터에 추가", type="primary"):
                    ...
                    if keep.empty: st.error("행이 없습니다.")
                    else:
                        with st.spinner("SMILES 조회 중..."): keep = LE.resolve_smiles(keep, verbose=False)
                        add_rows(keep)
...
with tab_form:
    ...
    with st.form("entry", clear_on_submit=False):
        ...
        ratio = f2.text_input("지질 몰비")
        g1, g2 = st.columns(2)
        ee = g1.number_input("EE (%)", 0.0, 100.0, 90.0, 0.1)
        ion = g2.text_input("이온화지질 이름")
        ok = st.form_submit_button("추가", type="primary")
    if ok:
        clean_doi = doi.strip().lower()
        if not clean_doi: st.error("DOI가 필요합니다.")
        elif not ratio.strip(): st.error("몰비가 필요합니다.")
        else:
            e = LE.Entry()
            e.paper(clean_doi)
            e.add(ratio.strip(), ee, ion=ion.strip() or None)
            new = e.to_frame()
            with st.spinner("SMILES 조회 중..."): new = LE.resolve_smiles(new, verbose=False)
            add_rows(new)

with tab_data:
    st.header("데이터 관리")
    F2.show_data_consistency(st, work_df, st.session_state.df)
    up2 = st.file_uploader("기존 CSV 불러오기", type=["csv"], key="csvup")
    if up2 is not None:
        try:
            d_raw = pd.read_csv(io.BytesIO(up2.read()), encoding="utf-8-sig")
            d_clean = pd.DataFrame(columns=st.session_state.df.columns)
            for col in st.session_state.df.columns:
                d_clean[col] = d_raw[col] if col in d_raw.columns else np.nan
            d_clean["encapsulation_efficiency_percent_std_num"] = d_clean["encapsulation_efficiency_percent_std_num"].map(P.robust_ee)
            if st.button("정상 데이터 추가하기"): add_rows(d_clean.dropna(subset=["encapsulation_efficiency_percent_std_num"]))
        except Exception as e: st.error(f"파싱 에러: {e}")
    st.divider()
    ed = st.data_editor(st.session_state.df, num_rows="dynamic", use_container_width=True, key="main_edit")
    if st.button("편집 내용 적용 심사", type="primary"):
        res = GD.screen_new_rows(ed.dropna(subset=["lipid_molar_ratio"]), _empty(), reduce_fn=AH.reduce_to_four)
...
with tab_model:
    ...
    if st.button("평가 실행", type="primary", disabled=(n_pap < 3)):
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("모델 MAE", f"{rep['mae_model']:.2f} %p", delta=f"{rep['gain_pct']:+.1f}% vs 평균만 답하기")
...
with tab_opt:
    if len(work_df) > 10:
        ...
        st.caption(base_model.transform_note)  # 💡 변환 스케일 선택 안내 출력
